...
export const hardwareSummary = (summary: JsonRecord, hardware: JsonRecord) => {
  ...
  const graphicsModel = firstText(controllers[0]?.model).trim();
  const graphicsVendor = firstText(controllers[0]?.vendor).trim();
  // Prefer the original model over summaries generated by older agents.
  const graphicsLabel = graphicsModel && (
    !graphicsVendor || graphicsModel.toLowerCase().startsWith(graphicsVendor.toLowerCase())
      ? graphicsModel
