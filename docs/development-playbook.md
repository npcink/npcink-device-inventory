...
产品的核心价值很窄且明确：管理员维护资产；Device Agent 上传电脑硬件事实；系统以可解释、可审计的规则匹配资产并保留观测与事件；管理端可以基于这些事实提供受约束的只读分析。它不是公开查询门户、ITSM、远程运维、工单或通用报表平台。
...
- 分析页按业务域组件拆分；每个用户可见区域只有一个渲染所有者，页面入口只保留查询、状态、导出和跨视图编排。具体边界见 [`ADR-015`](decisions/ADR-015-analysis-component-boundaries.md)。
...
- [`analysis-component-refactor-retrospective-2026-08-13.md`](analysis-component-refactor-retrospective-2026-08-13.md)：分析页组件化、重复渲染根因、收尾检查清单和可复用重构规范。
