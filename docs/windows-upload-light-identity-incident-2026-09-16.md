...
2026-09-16，78 第二次 RC 上传已核实，身份验收全部通过。管理员授权的六组历史记录（47、78、130、199、135、177）已在新备份和固定快照校验后用单事务整理：保留原编号及业务字段，迁入 8 条身份、12 条观测、18 条历史事件；归档 6 个重复条目，增加 12 条修复审计事件。事务内逐字段比对及事务后读取验证通过，修复前后备份已在服务器和本地校验 SHA-256，一次性远端执行工具已删除。135 原有“闲置”状态保留，不擅自改为使用中。

用户新增明确发布约束：必须先在本地确认外观，再明确同意，才能发布正式版本。因此 3.2.3 / 0.4.3 尚未发布；新优化交付 0.4.4-rc.1 预览，不创建正式 tag 或更新正式下载清单。
