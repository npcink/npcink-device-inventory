...
0.4.3-rc.1 已在 135、177 和有效 UUID 的 78 上完成上传、退出重开、再次上传。服务端核对确认：135、177 分别使用不同的永久 PCI MAC 身份，两次观测分别归属同一资产；78 使用已有系统 UUID 身份，并补齐主板和永久 PCI 网卡身份。三台互不串号。原始观测、资产对应关系和修复前备份保存在受限的本地运维目录，不进入仓库或发布包。

正式版使用 Agent 0.4.3 / 插件发布标签 3.2.3，功能与上述 RC 相同。`npm run build:release && npm run check:release` 已通过；本机 Docker daemon 未运行，未宣称完成 Docker 门。Windows RC 原生构建、fmt、Clippy 和测试见 Actions run 35065112602。正式发布仍需核对平台制品、签名和更新清单。

后续 0.4.4 单独补齐普通上传中的显卡/显示器/电池采集、完整硬件导出、同时显示显卡与显示器、隐藏无数据电池、身份自检和 Windows 系统工具入口。其改动不混入已实机验收的身份修复版本。
